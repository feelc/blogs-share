...
#define SAFE_RECV(sock, buf, buf_len, src_addr) \
    do \
    { \
...
void listen_cb(evutil_socket_t sock, short events, void* arg)
{
    ...
    while (1)
    {
        ...
        SAFE_RECV(conn_sock, buf, sizeof(buf), &client_addr);
        close(conn_sock);
        tlog[idx] += 1;
...
void *query_thread_cb(void *arg)
{
    ...
    pread_event = event_new(pbase, sock, EV_READ|EV_PERSIST, listen_cb, arg);
    if (NULL == pread_event)
    {
